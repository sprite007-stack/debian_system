...
section_6_set_monospace_font() {
  ...
  if run_as_user fc-list | grep -Fqi "MesloLGS Nerd Font Mono"; then
    chosen_font="MesloLGS Nerd Font Mono 14"
  elif run_as_user fc-list | grep -Fqi "MesloLGS Nerd Font"; then
    chosen_font="MesloLGS Nerd Font 14"
  elif run_as_user fc-list | grep -Fqi "Meslo"; then
    chosen_font="$(run_as_user fc-list | grep -i "Meslo" | head -n1 | sed 's/.*: \(.*\):.*//' | awk -F, '{print $1 " 14"}')"
  else
  ...
  run_as_user fc-list | grep -i "Meslo" | tee -a "$LOG_FILE" || true
