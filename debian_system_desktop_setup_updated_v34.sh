...
preseed_grub_pc() {
  local root_source="" parent_kname="" grub_disk=""

  root_source="$(findmnt -n -o SOURCE / 2>/dev/null || true)"
  if [[ -n "$root_source" ]]; then
    parent_kname="$(lsblk -no PKNAME "$root_source" 2>/dev/null | head -n1)"
    if [[ -z "$parent_kname" ]]; then
      parent_kname="$(lsblk -no KNAME "$root_source" 2>/dev/null | head -n1 | sed 's/[0-9]*$//')"
    fi
  fi

  if [[ -n "$parent_kname" ]]; then
    grub_disk="/dev/${parent_kname}"
  else
    grub_disk="$(lsblk -dno NAME,TYPE 2>/dev/null | awk '$2=="disk" {print "/dev/"$1; exit}')"
  fi
  ...
  printf '%s
' "grub-pc grub-pc/install_devices multiselect $grub_disk" | sudo debconf-set-selections >>"$LOG_FILE" 2>&1 || true
  printf '%s
' "grub-pc grub-pc/install_devices_empty boolean false" | sudo debconf-set-selections >>"$LOG_FILE" 2>&1 || true
...
sudo_run() {
  ...
  if $DRY_RUN; then
    info "[DRY-RUN] Skipping execution for: $desc"
    printf '[DRY-RUN CMD] sudo %q ' "$@" | tee -a "$LOG_FILE" >/dev/null
    echo | tee -a "$LOG_FILE" >/dev/null
    success "$desc"
    return 0
  fi
  if sudo DEBIAN_FRONTEND=noninteractive "$@" >>"$LOG_FILE" 2>&1; then
    success "$desc"
    return 0
  else
    error "$desc"
...
sudo_run_live() {
  ...
  if sudo DEBIAN_FRONTEND=noninteractive "$@" 2>&1 | tee -a "$LOG_FILE"; then
    echo "------------------------------------------------------------"
    success "$desc"
    return 0
  else
...
section_19_dev_tools_profile() {
  ...
  local reply=""
  local want_ansible=false
  local want_terraform=false
  ...
  echo
  warn "Node.js from Debian/LMDE repositories can pull in a very large dependency set, including npm and many supporting Node packages."
  warn "Choose the install mode you want for this system."
  while true; do
  ...
  if [[ "$node_mode" == "node_only" ]]; then
    warn "Node.js only will install the runtime from distro repositories without npm."
    if confirm "Proceed with Node.js only installation?" "Y"; then
      dev_tool_install_pkg nodejs "nodejs" "optional" || true
    else
      warn "Skipped Node.js installation."
    fi
  elif [[ "$node_mode" == "node_and_npm" ]]; then
    warn "Node.js + npm from distro repositories may install hundreds of packages on Debian/LMDE."
    if confirm "Proceed with Node.js + npm installation?" "Y"; then
      dev_tool_install_pkg nodejs "nodejs" "optional" || true
      dev_tool_install_pkg npm "npm" "optional" || true
    else
      warn "Skipped Node.js + npm installation."
    fi
  else
  ...
  if confirm "Install Ansible?" "Y"; then
    want_ansible=true
    selected_optional+=("Ansible")
    dev_tool_install_pkg ansible "ansible" "optional" || true
  fi

  if confirm "Install Terraform?" "Y"; then
    want_terraform=true
    selected_optional+=("Terraform")

    if pkg_installed terraform; then
      success "Already installed: terraform"
      already_installed+=("terraform")
    else
      info "Checking whether terraform is available from configured repositories..."
      if apt-cache policy terraform 2>/dev/null | awk '/Candidate:/ {print $2}' | grep -vq '(none)'; then
        dev_tool_install_pkg terraform "terraform" "optional" || true
      else
        warn "Terraform is not available from the currently configured distro repositories."
        warn "Add the official HashiCorp repository first if you want Terraform installed automatically."
        failed_optional+=("terraform (repo unavailable)")
        record_failure "optional:terraform-repo-unavailable"
      fi
    fi
  fi
  ...
  if confirm "Install kubectl from distro repositories if available?" "N"; then
    ...
    if pkg_installed kubectl; then
      ...
      if apt-cache policy kubectl 2>/dev/null | awk '/Candidate:/ {print $2}' | grep -vq '(none)'; then
        ...
        warn "kubectl is not available from the currently configured distro repositories."
        failed_optional+=("kubectl (repo unavailable)")
        record_failure "optional:kubectl-repo-unavailable"
      fi
    fi
  fi

  echo
  info "Optional development tool verification:"
  if $want_ansible; then
    verify_dev_tool "Ansible" ansible || true
  fi
  if $want_terraform; then
    verify_dev_tool "Terraform" terraform || true
  fi
  if $want_kubectl; then
    verify_dev_tool "kubectl" kubectl || true
  fi
  if $node_requested; then
    verify_dev_tool "Node.js" node || true
    if [[ "$node_mode" == "node_and_npm" ]]; then
      verify_dev_tool "npm" npm || true
    fi
  fi
  ...
  dev_tool_cmd_exists ansible && ansible --version | head -n1 | tee -a "$LOG_FILE"
  dev_tool_cmd_exists terraform && terraform version | head -n1 | tee -a "$LOG_FILE"
  ...
  echo
  section_result_summary "Section 19 - Dev Tools Profile" installed_now already_installed failed_required failed_optional

  if ((${#failed_required[@]} > 0)); then
    warn "Dev tools profile completed with required package warnings."
  elif ((${#failed_optional[@]} > 0)); then
    warn "Dev tools profile completed with optional package warnings."
...
run_all_sections() {
  ...
  if confirm "Run Section 21 - Validation Report?" "Y"; then run_section 21 section_21_validation_report; fi
}
...
show_menu() {
  ...
  format_menu_item 21 "Section 21 - Validation Report"
  echo " 22) Run ALL sections (prompt before each)"
  echo " 23) Exit"
  echo
}
...
main() {
  ...
  while true; do
    show_menu
    read -r -p "Select an option [0-23]: " choice
    ...
    case "$choice" in
      ...
      21) run_section 21 section_21_validation_report; press_enter ;;
      22) run_all_sections; press_enter ;;
      23) success "Exiting."; exit 0 ;;
