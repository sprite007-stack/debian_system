  ...
  if confirm "Install Nemo MediaInfo tab enhancement?" "Y"; then
    ...
    if run_as_user wget -O "$deb_file" "$deb_url" >>"$LOG_FILE" 2>&1; then
      if [[ -f "$deb_file" && -s "$deb_file" ]]; then
        success "Downloaded nemo-mediainfo-tab package"

        info "Installing nemo-mediainfo-tab..."
        if sudo dpkg -i "$deb_file" >>"$LOG_FILE" 2>&1; then
          success "Installed nemo-mediainfo-tab"
        else
          warn "dpkg failed, attempting dependency fix..."
          pkg_fix_broken
          if sudo dpkg -i "$deb_file" >>"$LOG_FILE" 2>&1; then
            success "Installed nemo-mediainfo-tab after fixing dependencies"
          else
            warn "Failed to install nemo-mediainfo-tab"
    ...
    warn "Skipping nemo-mediainfo-tab installation."
  fi
