...
run_as_user() {
  sudo -u "$STARTING_USER" env HOME="$STARTING_HOME" "$@"
}

run_as_user_shell() {
  sudo -u "$STARTING_USER" env HOME="$STARTING_HOME" bash -lc "$*"
}
...
section_6_set_monospace_font() {
  ...
  if fc-list | grep -Fqi "MesloLGS NF"; then
    chosen_font="MesloLGS NF 14"
  elif fc-list | grep -Fqi "MesloLGS Nerd Font"; then
    chosen_font="MesloLGS Nerd Font 14"
  elif fc-list | grep -Fqi "Meslo"; then
    chosen_font="$(fc-list | grep -i "Meslo" | head -n1 | sed 's/.*: \(.*\):.*/\1/' | awk -F, '{print $1 " 14"}')"
  else
    warn "Meslo font family was not detected by fontconfig."
    warn "Run Section 5 first, then refresh font cache and try again."
  ...
  if gsettings_key_exists org.gnome.desktop.interface monospace-font-name; then
    if run_as_user gsettings set org.gnome.desktop.interface monospace-font-name "$chosen_font" >>"$LOG_FILE" 2>&1; then
      success "Set org.gnome.desktop.interface::monospace-font-name to $chosen_font"
    else
      warn "Could not set org.gnome.desktop.interface::monospace-font-name"
      record_failure "gnome-monospace-font"
    fi
  else
    warn "GNOME monospace-font-name key is not available"
    record_failure "gnome-monospace-key"
  fi

  echo
  info "Installed Meslo-related fonts detected by fontconfig:"
  fc-list | grep -i "Meslo" | tee -a "$LOG_FILE" || true

  echo
  info "Verification:"
  echo "  GNOME monospace   : $(get_gsetting_if_available org.gnome.desktop.interface monospace-font-name)"
...
section_8_oh_my_zsh_and_plugins() {
  ...
  warn "Powerlevel10k wizard will run on first zsh start if ~/.p10k.zsh does not exist."
}
...
section_10_firewall() {
  ...
  warn "If you later publish Docker container ports, Docker can bypass expected UFW behavior."
}
...
section_12_nemo_enhancements() {
  ...
  local version="1.0.4"
  local deb_file="${DOWNLOAD_DIR}/nemo-mediainfo-tab_${version}_all.deb"
  local deb_url="https://github.com/linux-man/nemo-mediainfo-tab/releases/download/v${version}/nemo-mediainfo-tab_${version}_all.deb"
...
# ---------- Section 15 ----------
section_15_validation_report() {
  show_section_header "Section 15 - Validation Report"
...

  {
    ...
    echo "Command Checks"
    echo "--------------"
    for cmd in nala apt-get zsh flatpak docker ufw brave-browser 1password winboat; do
      if command_exists "$cmd"; then
    ...
    echo "APT Packages"
    echo "------------"
    for pkg in zsh tlp ufw gufw timeshift okular pdfarranger kodi 1password nfs-common; do
      if pkg_installed "$pkg"; then
        echo "$pkg: installed"
      else
