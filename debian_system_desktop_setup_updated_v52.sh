...
detect_userdb(){ HAS_SYSTEMD=0; USERDBD_INSTALLED=0; DM_NAME="unknown"; command -v systemctl >/dev/null 2>&1 && HAS_SYSTEMD=1; if [[ $HAS_SYSTEMD -eq 1 ]]; then for dm in sddm gdm gdm3 lightdm lxdm nodm; do systemctl is-active --quiet "${dm}.service" 2>/dev/null && { DM_NAME="$dm"; break; }; done; systemctl list-unit-files systemd-userdbd.service >/dev/null 2>&1 && USERDBD_INSTALLED=1; fi; }
...
EOF
  ...
  while true; do
    ...
    case "$choice" in
      1) sudo "$tool_path" --dry-run ;;
      2) sudo "$tool_path" --accept ;;
      3) sudo "$tool_path" --accept --persistent ;;
      4) sudo "$tool_path" --revert ;;
      5) [[ -f "$notes_file" ]] && sed -n '1,160p' "$notes_file" || warn "Notes file not found." ;;
      6) break ;;
