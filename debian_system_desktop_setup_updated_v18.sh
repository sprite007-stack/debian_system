...
section_14_jdownloader() {
  ...
  if [[ "$STARTING_USER" == "root" ]]; then
    ...
    warn "JDownloader should normally be installed for your desktop user, not root."
    warn "Consider launching the script with sudo from your normal user session."
  fi
  ...
  if [[ -n "${JDOWNLOADER_URL:-}" ]]; then
    ...
    if wget -O "$installer" "$JDOWNLOADER_URL" >>"$LOG_FILE" 2>&1; then
      success "Downloaded JDownloader installer"
    else
      error "Direct JDownloader download failed"
      record_failure "JDownloader-download"
      return 1
    fi
  else
    ...
    warn "Download the Linux installer manually to: $DOWNLOAD_DIR"
  fi

  if [[ -f "$installer" ]]; then
    chmod +x "$installer" >>"$LOG_FILE" 2>&1 || true
    ...
    warn "After downloading it manually, rerun Section 14."
  fi
}
